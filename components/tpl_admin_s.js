...
window.AppTemplates.adminSubject = `
<!-----------------科目活动开始 ----------------------------->
	...
	<div class="layout-container" id="view-admin" style="display:none;">
			...
			<div class="sidebar-container">
				<div class="sidebar-mode-switch">
					<div class="mode-switch-box">
						<div id="mode-btn-subject" onclick="switchViewMode('subject')" class="mode-btn active" >科目活动</div>
						<div id="mode-btn-global" onclick="switchViewMode('global')" class="mode-btn">全局活动</div>
					</div>
				</div>
				<div class="admin-tools-row">
					<select class="admin-select" id="admin_class_slc" onchange="onAdminClassChange()">
					...
					<div style="position:relative; flex:1; display:flex; align-items:center;">
						<input id="admin_search_inp" 
							   type="text" 
							   class="admin-search-input" 
							   placeholder="🔍 搜索活动" >
						
						<span id="admin_search_clear" 
							  class="admin-search-clear">
							✕
						</span>
					</div>
				</div>
				<div class="sidebar-tabs" id="admin_tab_container">
				...
				<div class="list-content" id="admin_activity_list" style="background:#FFF;">
					
				</div>

				<div class="add-btn-area">
					<button class="btn-add-dashed" onclick="createNewActivity()">+ 发布科目活动任务</button>
				</div>
			</div>
		</div>
		...
		<div class="right-panel panel-base" style="background: #FFFBF7;">
			<div class="panel-header" style="height: 50px; display: flex; align-items: center; justify-content: space-between; padding: 0 15px;">
				...
				<span id="header-default-title" style="font-size:16px; font-weight:800; color:#5D4037;">
					🛠 活动设置
				</span>
			...
			<div id="admin-action-area" class="action-area" style="background:transparent; display:none;">
				<div id="btn-mode-create">
					<button id="action-btns-subject" class="btn-full btn-save" onclick="saveAndExit()">新建科目活动</button>
				</div>
