...
function openDayConfig(dateStr) {
    if (window.isBatchMode) {
        toggleBatchCell(dateStr); // 下面会定义这个函数
        return; // 直接结束，不执行后面的弹窗代码
    }
	// 1. 获取数据源
    const uniqueKey = window.currentAdminActivityKey;
    if (!uniqueKey || !window.activityInfo[uniqueKey]) {
        alert("数据异常：未找到活动数据，请刷新重试！");
        return;
    }

    // ✅ 【核心修改】通过日期字符串反查 Index
    // 这样做是为了兼容 saveGridConfig (它依然依赖索引进行保存)
    const daysData = window.activityInfo[uniqueKey];
    const index = daysData.findIndex(d => d.date === dateStr);

    if (index === -1) {
        console.error("错误：未找到日期 " + dateStr);
        return;
    }

    console.log("打开配置: " + dateStr + " (索引: " + index + ")");

    // 2. 标记当前编辑的索引 (重要！保存函数 saveGridConfig 需要它)
    window.currentEditingDayIndex = index;
    window.isBatchMode = false; // 确保批量模式关闭
    ...
    // 解析班级名称
    let className = uniqueKey.split('_')[0];
    if (className === uniqueKey) className = className.trim();
    ...
    if (typeof toggleRestModeUI === 'function') {
        toggleRestModeUI(dayData.dayType === 0);
    }
	...
	// 1. 再次确认活动状态
    // 注意：window.currentAdminActivityKey 在上面已经有了，可以直接用
    const [cName, aName] = window.currentAdminActivityKey.split('_');
    const actItem = window.activityList.find(i => i.className === cName && i.activityName === aName);

    const saveBtn = document.querySelector('.dm-btn-save');

    if (saveBtn) {
        if (actItem && actItem.isEnd) {
            // -> 已结束：禁用保存按钮
            saveBtn.disabled = true;
            saveBtn.innerHTML = "活动已结束";
            saveBtn.style.backgroundColor = "#CCCCCC"; // 灰色背景
            saveBtn.style.cursor = "not-allowed";   // 禁止手势
        } else {
            // -> 进行中/未开始：恢复正常
            saveBtn.disabled = false;
            saveBtn.innerHTML = "💾保存配置";
            saveBtn.style.backgroundColor = ""; // 清空样式，使用CSS默认颜色(绿色/蓝色)
...
function openBatchConfig() {
    ...
    console.log(`打开批量配置，共选中 ${window.batchSelectedDates.size} 天`);
...
// --- 替换 logic_modal.js 中的 saveGridConfig 函数 ---
...
function saveGridConfig() {
    ...
    // ================== A. 从弹窗 DOM 中提取数据 (公用逻辑) ==================
    // 不管是单天还是批量，我们都先从界面上把配置读出来
    ...
    // 2. 如果是休息日，其他数据保持为空；如果是工作日/奖励日，提取详细数据
    if (newDayType !== 0) {
        // --- 提取奖品 ---
        if (newDayType === 2) {
            const rewardInputs = document.querySelectorAll('.dm-reward-box input[type="text"]');
        ...
        rows.forEach(row => {
            ...
            if (checkbox && checkbox.checked && subjKey) {
                ...
                inputs.forEach(input => {
                    ...
                    if (val) {
                        newTasks.push({ subject: subjKey, content: val });
                        hasCustomInput = true;
                    }
                });
                ...
                // 开关开了但没填内容，存占位符
                if (!hasCustomInput) {
                    newTasks.push({ subject: subjKey, content: "" });
                }
            }
        });
    ...
    if (window.isBatchMode) {
        ...
        console.log(`正在批量保存 ${window.batchSelectedDates.size} 天...`);
        ...
        window.batchSelectedDates.forEach(dateStr => {
            ...
            if (targetDay) {
                ...
                // 2. 赋值其他数据 (⚠️ 注意：必须使用 JSON 转换进行深拷贝，防止引用冲突)
                if (newDayType === 0) {
                    targetDay.tasks = [];
                    targetDay.exemptStudents = [];
                    targetDay.rewardData = null;
                } else {
                    targetDay.tasks = JSON.parse(JSON.stringify(newTasks));
