...
function refreshAdminView() {
    ...
    // 检查这个 Key 对应的卡片是否在当前列表中存在
    // (例如：如果从“进行中”切到“未开始”，原来的 Key 就不在列表里了，就需要重新选)
    ...
    if (currentKey && targetCard) {
        // ✅ 关键点：即使 Key 没变，也要强制调用 loadActivity
        // 这样才能修复 initDropdowns() 造成的下拉框重置问题
		
        loadActivity(currentKey); 
    } else {
		
        // 如果之前没选，或者选的那个不在当前列表里了，就自动选第一个
        autoSelectFirstItem();
    }
