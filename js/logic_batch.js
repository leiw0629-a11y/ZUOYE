...
/**
 * 提交批量录入
 * 1. 按照指定 JSON 结构写入 window.submissionData
 * 2. 将当前任务状态标记为已完成 (isBatch = true)
 * 3. 刷新界面
 */
function finishBatchEntry() {
    ...
    // --- 2. 获取任务详情 (用于填充 subject 和 task 字段) ---
    ...
    if (selectedCapsules.length === 0) {
	  ...
	  return; // 一定要 return，阻断后续逻辑
	}
	const selectedNames = Array.from(selectedCapsules).map(el => el.innerText.trim());
	
    // --- 4. 准备数据容器 (确保层级存在) ---
    // 确保 window.submissionData 存在
    if (!window.submissionData) {
        window.submissionData = {};
    }
    // 确保 第一层 key (act_id) 存在
    if (!window.submissionData[actId]) {
        window.submissionData[actId] = {};
    }
    // 确保 第二层 key (date) 存在
    if (!window.submissionData[actId][dateStr]) {
        window.submissionData[actId][dateStr] = []; // 初始化为空数组
    }

    // --- 5. 生成提交记录并写入 ---
    const now = new Date();
    // 生成格式化时间字符串: "2026-02-02 10:30:00"
    ...
    selectedNames.forEach(name => {
        const record = {
            "name": name,                               // [核心] 学生姓名
            "subject": currentTaskObj.subject,          // [核心] 科目
            "task": currentTaskObj.content || "",       // [核心] 细分任务 (无则空)
            "isLate": false,                            // [状态] 批量录入默认为按时 (false)
            "finishTime": timeStr,                      // [数据] 格式化后的时间字符串
            "note": "",                                 // [预留]
            "score": ""                                 // [预留]
        };
    ...
    // --- 6. 标记任务为“已录入”状态 ---
    // 注意：这里修改的是内存中的配置，标记该任务已完成
    currentTaskObj.isBatch = true; 

    // --- 7. 刷新界面 & 反馈 ---
    // 重新渲染左侧列表（让刚才的任务变绿、打钩）
    renderBatchSubjects(actId);
    
    // 提示成功 (可选，根据你的UI风格决定是否需要 alert)
	showToastHTML(`
		<div class="cm-toast-title">成功录入 ${selectedNames.length} 人</div>
	`);
	saveData();
    }
