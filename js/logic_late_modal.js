...
function getNormalLateStudents(className) {
    ...
    classActivities.forEach(activity => {
        ...
        actInfo.forEach(dayInfo => {
            ...
            const hasBatchTask = tasks.some(t => t.isBatch === true);
            ...
            classStudents.forEach(stu => {
                ...
                for (const task of tasks) {
                    ...
                    const hasSubmitted = daySubs.some(sub => {
                        ...
                        // 兼容旧数据：如果 sub.task 为空字符串，也算匹配
                        const subTask = sub.task || "";
                        return subTask === "" || subTask === content;
                    });
 ...
 * @param {Boolean} isSilent - 是否静默（批量时不弹窗，也不单独刷新）
 */
function handleLateSubmit(task, isSilent = false) {
    ...
    // 调试与保存
    console.log("写入成功:", newRecord);
    saveData(); 
    // ==========================================
    // 逻辑变更点：
    // 如果是单个点击（非静默），提交完立刻判断刷新逻辑
    // ==========================================
    if (!isSilent) {
		showToastHTML(`
